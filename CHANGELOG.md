...
All notable changes to Shelfie are documented here.
...
- **Home Assistant add-on install failed with `manifest unknown` (404).** The
  add-on pins `image: ghcr.io/amantux/shelfie` at the version in `config.yaml`,
...
- **Versioning & release automation.** CI auto-bumps the patch version in both
  `shelfie/config.yaml` and `custom_components/shelfie/manifest.json` on every
  push to `main`, keeping the add-on and integration versions in lockstep, then
  publishes `ghcr.io/amantux/shelfie:<version>` and `:latest`
  (linux/amd64 + linux/arm64 for Raspberry Pi 5).
- **Functional changelogs.** A root changelog (this file) plus a per-add-on
  `shelfie/CHANGELOG.md` that Home Assistant surfaces in the add-on store.
...
- **QR codes & barcodes**: printable Shelfie QR labels, plus register your own
...
[0.1.1]: https://github.com/Amantux/shelfie/releases/tag/0.1.1
[0.1.0]: https://github.com/Amantux/shelfie/releases/tag/0.1.0
